...
Cached = Class:extend
{
	...
	font = function(self,is_bitmap, options)


		local glyphs = options.glyphs or self.defaultGlyphs
		local size = options.size or 12

		-- this should somehow include glyphs.. in case 
		-- the same size/source file is used for a different set of glyphs 
		-- (that would be kind of silly... )
		local libKey = options.source..size

		if not self._library.font[libKey] then
			local font

			-- handle loading a bitmap font
			if is_bitmap then 
				local colorFormat
				if options.color_format and MOAIImage[options.color_format] then 
					colorFormat = MOAIImage[options.color_format]
				else
					colorFormat = MOAIImage.COLOR_FMT_RGBA_8888
				end

				font = MOAIFont.new()
				bitmapFontReader = MOAIBitmapFontReader.new ()
				bitmapFontReader:loadPage(options.source, glyphs, size)
				font:setReader( bitmapFontReader )

				glyphCache = MOAIGlyphCache.new ()
				-- TODO: make this user adjustable
				glyphCache:setColorFormat (colorFormat)
			...
			-- handle loading ttf
				...
				local dpi = options.dpi or 72

				font = MOAIFont.new()
				font:loadFromTTF(options.source, glyphs, size, dpi)

				--[[
				switch the above ^ to use
					font:load('path/to/file.ttf')
		...
		if not self._library.font[libKey] then
			...
			if #arg == 1 then
				...
				elseif type(arg[1]) == 'string' then
					font = MOAIFont.new()
					bitmapFontReader = MOAIBitmapFontReader.new ()
					bitmapFontReader:loadPage (arg[1], charcodes, 18 )
					font:setReader ( bitmapFontReader )
